...
if (-not $env:DevEnvDir) {
    ...
    if (-not (Test-Path $vsDevShell)) {
        throw "Could not find Launch-VsDevShell.ps1. Install Visual Studio 2022 with C++ build tools."
    }
    Push-Location
...
function SignZedAndItsFriends {
    ...
    $editorExe = if (Test-Path "$innoDir\ZedRail.exe") { "ZedRail.exe" } else { "Zed.exe" }
    $files = "$innoDir\$editorExe,$innoDir\cli.exe,$innoDir\auto_update_helper.exe,$innoDir\zed_explorer_command_injector.dll,$innoDir\zed_explorer_command_injector.appx"
...
function BuildInstaller {
    ...
    switch ($channel) {
        "stable" {
            ...
            $appName = "ZedRail"
            $appDisplayName = "ZedRail"
            $appSetupName = "ZedRail-$Architecture"
            # The mutex name here should match the mutex name in crates\zed\src\zed\windows_only_instance.rs
            $appMutex = "ZedRail-Editor-Stable-Instance-Mutex"
            $appExeName = "ZedRail"
            $regValueName = "ZedRail"
            $appUserId = "Qu3ntinS.ZedRail"
            $appShellNameShort = "Zed&Rail"
            $appAppxFullName = "Qu3ntinS.ZedRail_1.0.0.0_neutral__japxn1gcva8rg"
        }
        "preview" {
            $appId = "{{F70E4811-D0E2-4D88-AC99-D63752799F95}"
            $appIconName = "app-icon-preview"
            $appName = "ZedRail Preview"
            $appDisplayName = "ZedRail Preview"
            $appSetupName = "ZedRail-$Architecture"
            # The mutex name here should match the mutex name in crates\zed\src\zed\windows_only_instance.rs
            $appMutex = "ZedRail-Editor-Preview-Instance-Mutex"
            $appExeName = "ZedRail"
            $regValueName = "ZedRailPreview"
            $appUserId = "Qu3ntinS.ZedRail.Preview"
            $appShellNameShort = "Zed&Rail Preview"
            $appAppxFullName = "Qu3ntinS.ZedRail.Preview_1.0.0.0_neutral__japxn1gcva8rg"
        }
        "nightly" {
            $appId = "{{1BDB21D3-14E7-433C-843C-9C97382B2FE0}"
            $appIconName = "app-icon-nightly"
            $appName = "ZedRail Nightly"
            $appDisplayName = "ZedRail Nightly"
            $appSetupName = "ZedRail-$Architecture"
            # The mutex name here should match the mutex name in crates\zed\src\zed\windows_only_instance.rs
            $appMutex = "ZedRail-Editor-Nightly-Instance-Mutex"
            $appExeName = "ZedRail"
            $regValueName = "ZedRailNightly"
            $appUserId = "Qu3ntinS.ZedRail.Nightly"
            $appShellNameShort = "Zed&Rail Nightly"
            $appAppxFullName = "Qu3ntinS.ZedRail.Nightly_1.0.0.0_neutral__japxn1gcva8rg"
        }
        "dev" {
            $appId = "{{8357632E-24A4-4F32-BA97-E575B4D1FE5D}"
            $appIconName = "app-icon-dev"
            $appName = "ZedRail Dev"
            $appDisplayName = "ZedRail Dev"
            $appSetupName = "ZedRail-$Architecture"
            # The mutex name here should match the mutex name in crates\zed\src\zed\windows_only_instance.rs
            $appMutex = "ZedRail-Editor-Dev-Instance-Mutex"
            $appExeName = "ZedRail"
            $regValueName = "ZedRailDev"
            $appUserId = "Qu3ntinS.ZedRail.Dev"
            $appShellNameShort = "Zed&Rail Dev"
            $appAppxFullName = "Qu3ntinS.ZedRail.Dev_1.0.0.0_neutral__japxn1gcva8rg"
        }
        default {
